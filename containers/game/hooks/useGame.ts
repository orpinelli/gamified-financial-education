...
import { GAME_EVENTS, getRandomEvent, isSalaryDay, SALARY_EVENT } from "@/data/events";
...
function getEventForDay(day: number, professionId: string, stats: { money: number; knowledge: number; happiness: number; energy: number; health: number }): GameEvent {
  const profession = PROFESSIONS.find((p) => p.id === professionId);
  if (!profession) return GAME_EVENTS[0];

  // Salary day
  if (isSalaryDay(day)) {
    const salary = calculateSalary(profession, stats.knowledge);
    const salaryEvent: GameEvent = {
      ...SALARY_EVENT,
      id: `pagamento_salario_${day}`,
      description: `Hoje e dia de receber seu salario como ${profession.name}. Valor: R$${salary.toFixed(2)}`,
      options: [
        {
...
export function useGame() {
  ...
  const triggerEvent = useCallback(() => {
    ...
    const event = getEventForDay(Number(session.current_day), session.profession_id, stats);
